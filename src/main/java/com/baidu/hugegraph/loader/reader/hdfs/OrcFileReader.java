...
public class OrcFileReader extends HDFSReader {
    ...
    public void init() {
        try {
            this.reader = OrcFile.createReader(this.fileSystem, this.path);
            this.inspector = (StructObjectInspector) this.reader.getObjectInspector();
            List<? extends StructField> fields = this.inspector.getAllStructFieldRefs();
        ...
        if (this.nextLine == null) {
            try {
                ...
                if (row != null) {
                    ...
                    this.nextLine = new Line((String) row, this.names, values);
                } else {
                    this.nextLine = null;
                }
            } catch (IOException e) {
