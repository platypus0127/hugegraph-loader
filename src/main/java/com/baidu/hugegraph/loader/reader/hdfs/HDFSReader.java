...
public class HDFSReader extends FileReader {
    ...
    @Override
    protected InputStream openInputStream(FileSource source) throws IOException {
        LOG.info("Opening HDFS file {}", source.path());
        URI hdfsUri = URI.create(source.path());
        try {
            this.fileSystem = FileSystem.get(hdfsUri, this.conf);
        } catch (IOException e) {
            throw new LoadException("Failed to create load file system", e);
        }

        this.path = new Path(source.path());
        checkPath(this.path, this.fileSystem);

        return this.fileSystem.open(this.path);
    }
